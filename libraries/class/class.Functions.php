...
class Functions
{
    ...
    /* Get news */
    public function getNewsItem($news = array())
    {

        $str = '';
        if (!empty($news)) {
            /* Data */
            $news['target'] = 'target="_blank"';
            /* Params for markdown */
            $params = array();
            $params['news'] = $news;
            /* Get markdown */
            $str = $this->markdown('news/news', $params);
        }
        return $str;
    }
    ...
    {
        // Tách phần tên người dùng từ URL
        $parts = explode('@', $url);
        $username = end($parts);
        // Tạo mã iframe
        $endcode = '<blockquote class="tiktok-embed p-0 m-0 d-inline-block" cite="https://www.tiktok.com/' . $username . '" data-unique-id="' . $username . '" data-embed-type="creator" style="max-width: 500px; min-width: 288px;height: ' . $height . 'px;" > <section> <a target="_blank" href="https://www.tiktok.com/' . $username . '?refer=creator_embed">' . $username . '</a> </section> </blockquote> <script async src="https://www.tiktok.com/embed.js"></script>';
        return $endcode;
    }

    public function getFontNames($css)
    {
        $fontNames = [];
