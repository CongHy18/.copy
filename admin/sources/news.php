    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('news', $data)) {
            ...
            if ($func->hasFile("file2")) {
                ...
                if ($photo = $func->uploadImage("file2", $config['news'][$type]['img_type'], UPLOAD_NEWS, $file_name)) {
...
function deleteMan()
{
    ...
    if ($id) {
        /* Lấy dữ liệu */
        $row = $d->rawQueryOne("select id, photo, photo2, file_attach from #_news where id = ? and type = ? limit 0,1", array($id, $type));
