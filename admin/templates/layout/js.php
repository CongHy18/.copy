<?php 
    // Thêm font vào admin
    $cssContent = file_get_contents("../assets/css/fonts.css");
    $fontNames = $func->getFontNames($cssContent);
    $arrFont = array_map(function($font) {
        return $font . '/' . $font . ';'; 
    }, $fontNames);
    $fontNews = implode(' ', $arrFont);
?>

<!-- Js Config -->
<script type="text/javascript">
    // var PHP_VERSION = parseFloat('<?php /* phpversion()*/ ?>'.replaceAll('.', ','));
    ...
    // My style 
    var image2 = '';if(<?= (!empty($config['LQD']['image2'])) ? 'true' : 'false' ?>){image2 =',image2'};
    var FONTS_CKF = '<?= $fontNews ?>';
</script>
