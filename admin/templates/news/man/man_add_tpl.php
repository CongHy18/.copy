                                    ...
                                    <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['desc']) && $config['news'][$type]['desc'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="desc<?= $k ?>"><?php echo ($type == 'chi-nhanh') ? 'Iframe bản đồ' : 'Mô tả (' . $k . ') '; ?> :</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc_cke']) && $config['news'][$type]['desc_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="<?php echo ($type == 'chi-nhanh') ? 'Mã nhúng iframe' : 'Mô tả (' . $k . ') '; ?>"><?= $func->decodeHtmlChars($flash->get('desc' . $k)) ?: $func->decodeHtmlChars(@$item['desc' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
